...
def test_handle_error_traceback_suppression():
    async def run_test():
        ...
        with patch(
            "research_cli.researcher.escape_markup",
            ...
            with patch.dict(os.environ, {}, clear=True):
                with patch(
                    ...
                    await agent._handle_error(
                        task_id=1,
                        prefix="Error",
                        db_msg="Sensitive Error Message [bold]markup[/bold]",
                    )

            # Check that escape_markup was called
            mock_escape.assert_called_once_with(
                "Sensitive Error Message [bold]markup[/bold]"
            )
    ...
    async def run_test():
        ...
        with patch.dict(os.environ, {"RESEARCH_DEBUG": "1"}):
            with patch(
                ...
                await agent._handle_error(
                    task_id=1, prefix="Error", db_msg="Error Message"
                )
